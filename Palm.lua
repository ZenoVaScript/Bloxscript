...
local function toggleSpeedBoost(state)
    ...
    if state then
        ...
        humanoid.WalkSpeed = 60 -- Set to 60 as requested
        ...
        -- Start position checking to prevent teleport-back
        if positionCheckConnection then positionCheckConnection:Disconnect() end
...
local function teleportToSky()
    ...
    if door and root then
        lastPosition = door + Vector3.new(0, 200, 0)
...
local function teleportToGround()
    if not root then updateCharacter() end
    if root then
        lastPosition = root.Position - Vector3.new(0, 50, 0)
        root.CFrame = CFrame.new(lastPosition)
    end
end
