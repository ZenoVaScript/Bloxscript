...
local ActiveSpeedBoost,ActiveAutoUseCoinFlip,ActiveEspSurvivors,ActiveNoStun,ActiveEspKillers,ActiveEspGenerator,ActiveEspItems,ActiveInfiniteStamina,ActiveEspRagdolls,ActiveAutoGenerator,AutoKillSurvivors,RemoveLags,ActiveBackstab = false,false,false,false,false,false,false,false,false,false,false,false,false
...
-- Added Backstab V2 Toggle
local BackstabToggle = PlayerTab:CreateToggle({
   Name = "Backstab V2",
   CurrentValue = false,
   Flag = "BackstabV2",
   Callback = function(Value)
      ActiveBackstab = Value
      if ActiveBackstab then
         -- Load the backstab script when toggled on
         ...
         -- You might want to add code here to properly unload/destroy the backstab script
         -- This depends on how the backstab script is structured
         Rayfield:Notify({
            Title = "Backstab V2",
            Content = "Turn off the backstab feature in its own GUI",
            Duration = 3,
            Image = "warning"
         })
      end
