...
local Window = Rayfield:CreateWindow({
   ...
   ConfigurationSaving = {
      Enabled = true,
	      FolderName = "SaverForsaken", -- Create a custom folder for your hub/game
      FileName = "K"
   },
...
Rayfield:Notify({
   Title = "Cheat Version",
   Content = "V.0.37 | Thank you for using this script 🎉 ",
   Duration = 2.5,
   Image = "rewind",
})
local Paragraph1 = InfoTab:CreateParagraph({Title = "TP To Generator", Content = "This Function Work Now And If You Got Kicked I Guess Sorry!"})
local Paragraph2 = InfoTab:CreateParagraph({Title = "Fixed Full Bright (V.0.23)", Content = "Normally Full Bright Is Fixed Try It!"})
local Paragraph3 = InfoTab:CreateParagraph({Title = "Fixed Items (V.0.24)", Content = "Baru Diperbaiki Sekarang!"})
local Paragraph4 = InfoTab:CreateParagraph({Title = "error function", Content = "If there is an error in the function, please contact the script developer on Discord.!"})
...
if Players:isA("Model") and (Players.Name == "1x1x1x1" or Players.Name == "Jason") and Players:FindFirstChild("Head") and Players:FindFirstChildOfClass("Highlight") and not Players.Head:FindFirstChildOfClass("BillboardGui") then
...
local DiscordLink = DiscordTab:CreateButton({
   Name = "Discord Link",
   Callback = function()
copyToClipboard("https://discord.gg/E2TqsRP4")
end,
})
